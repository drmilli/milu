        ...
        <div className="relative lg:hidden" ref={bizMenuRef}>
          <button
            ...
            className={clsx(
              ...
              user?.role === 'OWNER' ? 'hover:bg-cream cursor-pointer' : 'cursor-default'
            )}
          >
            ...
            <div className="text-left hidden xs:block">
              <p className="text-sm font-semibold text-primary-dark leading-tight truncate max-w-[120px]">
                {user?.businessName || '—'}
              </p>
              <p className="text-xs text-primary-warm leading-tight capitalize">{planLine}</p>
            </div>
            {user?.role === 'OWNER' && (
              <svg className="w-3 h-3 text-primary-warm flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
